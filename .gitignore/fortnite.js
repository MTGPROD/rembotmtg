...
exports.run = async (bot, message, args) => {
    ...
    if (args[2]) {
        if (args[2].toLowerCase() == "all" || args[2].toLowerCase() == "season") {
            ...
            return message.channel.send(
                "Error. Use the right syntax: `r!fortnite <epic-username> [platform pc/xbl/psn] {mode all/season}`.\nFor lifetime stats use `r!fortnite <epic-username> [platform pc/xbl/psn]`"
            ); //Sends error message
        }
    }
    ...
    if (!username)
        ...
        return message.channel.send(
            "Username not provided. Use the right syntax: `r!fortnite <epic-username> [platform pc/xbl/psn] {mode all/season}`.\nFor lifetime stats use `r!fortnite <epic-username> [platform pc/xbl/psn]`"
        ); //Sends error message
    ...
    let data = ftnApi
        ...
        .catch(e => {
            ...
            //console.log(e);
            return message.channel.send(
                "Error. User not found\n```\nUsage: r!fortnite <epic-username> [pc | xbl | psn] {all | season}\nExample: r!fornite Gotaga pc```"
            ); //Send error message
        });
...
exports.help = {name: "fortnite", aliases: []} 