...
exports.run = (bot, message, args) => {
 ...
 /* if(args[0] === 'set') {
  ...
  if(!target) return message.channel.send('`Usage: r!unmute @user [reason]`')
  ...
  if(!target.roles.has(guild[message.guild.id].muteRole)) return message.channel.send("This member is already muted.")  
  target.removeRoles([`${guild[message.guild.id].muteRole}`])
  message.channel.send({embed: {
    description: `${target.id} is now unmuted ! Reason: ${reason}` 
  }})
} 
